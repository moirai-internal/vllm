...
@_nvml()
def get_gpu_memory(devices: list[int]):
...
def create_new_process_for_each_test(method: str = "fork") -> Callable:
    """Decorator factory to create a new process for each test function.
    ...
    def decorator(f: Callable[_P, None]) -> Callable[_P, None]:
        ...
        def wrapper(*args: _P.args, **kwargs: _P.kwargs) -> None:
            if method == "spawn":
                ...
                # Use subprocess module instead of multiprocessing
                import subprocess
                import sys

                cmd = [
                    sys.executable, "-c", f"import {module_name}; " +
                    f"{module_name}.{func_name}(*{args}, **{kwargs})"
                ]
...
# TODO: Clean up to use create_new_process_for_each_test
