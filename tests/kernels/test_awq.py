...
def test_awq_dequantize_opcheck(monkeypatch: pytest.MonkeyPatch, dtype_str: str):
    with monkeypatch.context() as m:
        ...
        qweight = torch.randint(-2000000000,
                                ...
                                device='cuda',
                                dtype=torch.int32)
        scales = torch.rand((64, 2048), device='cuda', 
        ...
        opcheck(torch.ops._C.awq_dequantize,
                (qweight, scales, zeros, split_k_iters, thx, thy, dtype_str))
...
@pytest.mark.skipif(not hasattr(torch.ops._C, "awq_gemm"),
                    reason="AWQ is not supported on this GPU type.")
@pytest.mark.parametrize("dtype_str", ["fp16", "bf16"])
def test_awq_gemm_opcheck(monkeypatch: pytest.MonkeyPatch, dtype_str: str):
    with monkeypatch.context() as m:
        m.setenv("VLLM_USE_TRITON_AWQ", "0")
        input = torch.rand((2, 8192), device='cuda',
                          dtype=torch.bfloat16 if dtype_str == "bf16" else torch.float16)
        qweight = torch.randint(-2000000000,
                                2000000000, (8192, 256),
                                device='cuda',
        ...
        scales = torch.randint(-2000000000,
                               ...
                               device='cuda',
                               dtype=torch.int32)
        qzeros = torch.empty((64, 2048), device='cuda', 
                           dtype=torch.bfloat16 if dtype_str == "bf16" else torch.float16)
        split_k_iters = 8
